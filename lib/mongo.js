...
const { addDeleted } = Package['jam:soft-delete'] || {};
 ...
 * @param {Object} [filter=Offline.config.filter] - The optional filter object to keep.
 ...
 */
Mongo.Collection.prototype.keep = function(filter = Offline.config.filter, { sort = Offline.config.sort, limit = Offline.config.limit } = {}) {
  if (deepContains(filter, null)) filter[needsReplace] = true;
  if (addDeleted) addDeleted(filter);

  offlineCollections.set(this._name, { filter, sort, limit }); // this._name is the collection name
  return;
}
...
Mongo.Collection.prototype.clear = async function() {
  ...
  return clear(this._name); // this._name is the collection name
}
