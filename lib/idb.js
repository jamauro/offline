...
import { bc } from './bc';
import { wait } from './utils/client';

const openDB = () => open('db', 1, {
  upgrade(db) {
    for (const [ name, { filter, sort, limit } ] of offlineCollections) {
...
export async function put(name, doc, { added = false } = {}) {
  ...
  const isOffline = status !== 'connecting' && !connected;
...
export async function queueMethod(name, ...args) {
  ...
  const db = await openDB();
  return db.add('methods', { name, args });
}
