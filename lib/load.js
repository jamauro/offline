...
async function load() {
  ...
  const loadAndObserve = async (localCollection, filter) => {
    ...
    if (isEmpty(filter) || Meteor.userId()) {
  ...
  try {
    await Promise.all(
      [...offlineCollections].map(([ name, { filter } ]) => loadAndObserve(localCollections[name], filter))
    );
  } catch (error) {
