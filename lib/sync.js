      ...
      for (const { id, name, args: mArgs, collectionName, docId } of methods) {
        ...
        try {
          ...
          if (docId) { // if we have a docId, it means that it was added while offline. this docId is temporary and is only in the localCollection.
            ...
            await updateMethods(methods, docId, stringifyId(result)).catch(error => console.error(error => 'updateMethods error', error)); // we'll want to update any changes made to that document with the real _id after it's inserted on the server. this assumes that the _id is returned whenever a document is inserted which is fairly standard. we don't want these errors to be included in methodErrors.
          }
        } catch (error) {
